...
# --- 2. インタラクティブ・分析 (中央：フィルター・ズーム・クリック) ---
st.divider()
st.subheader("🔍 インタラクティブ・分析")
st.caption("Ctrl+Wheel：ズーム / ドラッグ：移動 / 点をクリック：Giftメッセージ表示")

if os.path.exists(json_path):
    all_colors = sorted(list(set([n['color'] for n in anim_data['nodes']])))
    selected_colors = st.multiselect("表示する色のカテゴリーを選択（空だと全表示）", options=all_colors, default=[])
